...
public class CardDto {
	...
	public String toString() {
		return "카드 정보(CardDto) [카카오페이 매입사명(purchase_corp): " +getPurchase_corp() + ", 카카오페이 매입사 코드(purchase_corp_code): " + getPurchase_corp_code()
			+ ", 카카오페이 발급사명(issuer_corp): " + getIssuer_corp() + ", 카카오페이 발급사 코드(issuer_corp_code): " + getIssuer_corp_code()
			+ ", 카드 BIN(bin): " + getBin() + ", 카드 타입(card_type): " + getCard_type()
			+ ", 할부 개월 수(install_month): " + getInstall_month() + ", 카드사 승인번호(approved_id): " + getApproved_id()
			+ ", 카드사 가맹점 번호(card_mid): " + getCard_mid() + ", 무이자할부 여부(Y/N)(interest_free_install): " + getInterest_free_install()
			+ ", 카드 상품 코드(card_item_code): " + getCard_item_code() + "]";
	}
